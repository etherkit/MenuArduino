 ...
 *
 * 28 January 2018
 * Some minor modifications by Jason Milldrum in order to support the iterator
 * functions begin_elem and end.
 */
 ...
 public:
     ...
     {
         begin_elem = new VectorType[initialSize]; //points to the beginning of the new array
         head = initialSize - 1;
         storage = begin_elem + initialSize; //points to the element one outside of the array (such that end - begin_elem = capacity)
     }

     Vector(Vector &obj)
     {
         begin_elem = new VectorType[0]; // Points to the beginning of the new array, it's zero but this line keeps malloc from seg faulting should we delete begin_elem before resizing it
         head = -1;
         storage = begin_elem; //points to the element one outside of the array (such that end - begin_elem = capacity)

         *this = obj;
     }

     // If there's anything in the vector then delete the array, if there's no array then doing will will cause seg faults
     virtual ~Vector() { delete[] begin_elem; }
     ...
     void Swap(Vector &obj)
     {
         ...
         SWAP(VectorType*, begin_elem, obj.begin_elem);
     ...
     void PushBack(const VectorType *elements, int len)
     {
         ...
         int append = MIN(storage - begin_elem - head - 1, len), prepend = len - append;

         // memcpy the data starting at the head all the way up to the last element *(storage - 1)
         memcpy((begin_elem + head + 1), elements, sizeof(VectorType) * append);

         // If there's still data to copy memcpy whatever remains, starting at the first element *(begin_elem) until the end of data. The first step will have ensured
         // that we don't crash into the tail during this process.
         memcpy(begin_elem,(elements + append), sizeof(VectorType) * prepend);
     ...
     void Erase(int first, int last)
     {
         ...
         for(int i = 0; i < (Size() - first); i++)
         {
             ...
             begin_elem[first + i] = begin_elem[last + i];
         }
     ...
     VectorType const &Back() { return *begin_elem; }

     // Returns the newest element in the array (the one added after every other)
     VectorType const &Front() { return begin_elem[head]; }
     ...
     VectorType *Data() { return begin_elem; }
     ...
     void Assign(int len, const VectorType &val)
     {
         ...
         begin_elem = new VectorType[len];
         storage = begin_elem + len;
     ...
     void Assign(VectorType *array, int len)
     {
         ...
         memcpy(begin_elem, array, sizeof(VectorType) * len);
     }

     // Returns the number of elements that the vector will support before needing resizing
     int Capacity() { return (storage - begin_elem); }
